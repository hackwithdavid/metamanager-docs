...
const payload = {
  ...
  tn: 'customer-42',                          // which brand to use (optional)
...
Register a brand once per customer, then name it with `tn` in the payload:

```bash
curl -X PUT https://metamanager.dev/api/og/tenants/customer-42 \
  -H 'content-type: application/json' \
  -d '{"background":"#0b0c0d","text":"#f4f5f6","accent":"#ff0055","template":"split"}'
```
