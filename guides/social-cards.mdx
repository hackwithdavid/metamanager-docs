...
up to 4 MB &mdash; enough for an unoptimised photograph straight from a stock
library.
...
<Note>
  A card whose image cannot be fetched &mdash; wrong address, too large, too slow,
  not an image &mdash; draws without it rather than failing, and says why in an
